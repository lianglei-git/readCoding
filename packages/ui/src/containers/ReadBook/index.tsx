...
const ReadBookContainer = () => {
    ...
    return <div className="ReadBookContainer">
        <div className="ReadBookContainer__tab">
            <SpButton type={active == 'bookShelf' ? 'primary': 'default'} onClick={() => onClick('bookShelf')}>书架</SpButton>
            <SpButton type={active == 'readFile' ? 'primary': 'default'} onClick={() => onClick('readFile')}>读书</SpButton>
            <SpButton type={active == 'upload' ? 'primary': 'default'}  onClick={() => onClick('upload')}>上传</SpButton>
        </div>
        {active =='bookShelf' && <BookShelf BookShelfDoubleClick={BookShelfDoubleClick} />}
